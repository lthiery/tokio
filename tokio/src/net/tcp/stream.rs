...
impl TcpStream {
    ...
    /// to the first I/O operation on the returned `TcpStream`, so this
    /// constructor itself does not panic and may be called from any
    /// thread; the panic moves to the first `.readable()` /
    /// `.writable()` / read / write call.
    #[track_caller]
...
cfg_io_uring_reactor! {
    use crate::runtime::io::uring_bytes_ops::{CompleterShared, UringRecv, UringSend};
    use crate::runtime::io::uring_driver::{local_reactor_installed, with_local_reactor};
    use crate::runtime::io::uring_recv_multi::{RecvInbox, UringRecvMulti};

    impl TcpStream {
        /// Submit an owned-buffer `send()` directly to the per-worker
        /// `io_uring` reactor.
        ///
        /// The kernel reads from `buf` for the duration of the SQE, so
        /// the buffer is held inside the reactor until the op completes
        /// — the caller gets it back (with the kernel's byte count)
        /// via the returned [`UringSend`] future. The buffer is
        /// returned on both success and error paths, so it can be
        /// reused without an extra allocation.
        ///
        /// # Cancellation safety
        ///
        /// Dropping the returned future does **not** immediately free
        /// the buffer. The reactor best-effort submits an
        /// `AsyncCancel`, and the buffer is released only when the
        /// terminal CQE arrives (either the natural completion or
        /// `-ECANCELED`). Memory safety is upheld regardless — the
        /// kernel can never outrun the buffer's lifetime.
        ///
        /// # Errors
        ///
        /// The future resolves with `(Err, buf)` if:
        ///
        /// * The current task is not running on a worker with the
        ///   uring reactor enabled (`ErrorKind::Unsupported`). In this
        ///   case `buf` is the *original* buffer the caller passed.
        /// * The SQE could not be pushed onto the submission ring
        ///   (surface of the push error). `buf` is again the original.
        /// * The kernel posts a negative `send()` result (surfaced via
        ///   `io::Error::from_raw_os_error`).
        ///
        /// # Platform support
        ///
        /// Linux only, requires `--cfg tokio_unstable` and the
        /// `io-uring-reactor` feature.
        ///
        /// [`UringSend`]: crate::runtime::io::uring_bytes_ops::UringSend
        #[cfg_attr(docsrs, doc(cfg(all(tokio_unstable, feature = "io-uring-reactor", target_os = "linux"))))]
        pub fn uring_send(&self, buf: bytes::Bytes) -> UringSend {
            use std::os::fd::AsRawFd;
            // Probe first — `with_local_reactor` consumes its closure
            // by move, so we'd lose `buf` on the not-a-worker branch
            // if we called it unconditionally.
            if !local_reactor_installed() {
                return UringSend::already_complete_err(
                    io::Error::new(
                        io::ErrorKind::Unsupported,
                        "uring_send requires a tokio worker with the \
                         io-uring-reactor enabled",
                    ),
                    buf,
                );
            }
            ...
            // Unwrap is safe: we just probed and a reactor is installed.
            // It cannot be uninstalled between the probe and the call
            // because both happen on this thread without a yield point.
            let submit = with_local_reactor(|reactor| {
                reactor.submit_send_bytes(fd, buf, shared.clone())
            })
            .expect("local reactor vanished between probe and submit");
            match submit {
                Ok(user_data) => UringSend::new(shared, user_data),
                Err((err, recovered)) => UringSend::already_complete_err(err, recovered),
            }
        }

        /// Submit an owned-buffer `recv()` directly to the per-worker
        /// `io_uring` reactor.
        ///
        /// The kernel writes into `buf` (up to `buf.len()` bytes) for
        /// the duration of the SQE. The caller gets the buffer back
        /// (with the kernel's byte count, which may be less than the
        /// capacity on short reads) via the returned [`UringRecv`]
        /// future.
        ///
        /// `buf.len()` defines the receive capacity; callers wishing
        /// to receive into a larger window should [`resize`] or
        /// [`reserve`] + [`set_len`] beforehand.
        ///
        /// See [`Self::uring_send`] for cancellation-safety, buffer-
        /// return and error semantics (they mirror exactly).
        ///
        /// [`UringRecv`]: crate::runtime::io::uring_bytes_ops::UringRecv
        /// [`resize`]: bytes::BytesMut::resize
        /// [`reserve`]: bytes::BytesMut::reserve
        /// [`set_len`]: bytes::BytesMut::set_len
        #[cfg_attr(docsrs, doc(cfg(all(tokio_unstable, feature = "io-uring-reactor", target_os = "linux"))))]
        pub fn uring_recv(&self, buf: bytes::BytesMut) -> UringRecv {
            use std::os::fd::AsRawFd;
            if !local_reactor_installed() {
                return UringRecv::already_complete_err(
                    io::Error::new(
                        io::ErrorKind::Unsupported,
                        "uring_recv requires a tokio worker with the \
                         io-uring-reactor enabled",
                    ),
                    buf,
                );
            ...
            let shared = CompleterShared::new();
            let submit = with_local_reactor(|reactor| {
                reactor.submit_recv_bytes(fd, buf, shared.clone())
            })
            .expect("local reactor vanished between probe and submit");
            match submit {
                Ok(user_data) => UringRecv::new(shared, user_data),
                Err((err, recovered)) => UringRecv::already_complete_err(err, recovered),
            }
        }

        /// Arm a multishot `recv()` on this socket backed by the
        /// reactor's provided-buffer ring.
        ///
        /// One io_uring SQE stays armed on the socket for the entire
        /// lifetime of the returned [`UringRecvMulti`]; each delivered
        /// message is a kernel-chosen buffer from the ring, surfaced
        /// to user code as a
        /// [`BufferLease`](crate::runtime::io::uring_buf_ring::BufferLease)
        /// that the caller reads from and drops to recycle.
        ///
        /// This path avoids the per-recv SQE and per-recv slab entry
        /// that [`Self::uring_recv`] pays; it is the io_uring native
        /// analogue of the readiness-mode "poll once, read many" loop.
        ///
        /// # Cancellation
        ///
        /// Dropping the [`UringRecvMulti`] submits a best-effort
        /// `AsyncCancel2` matching the socket fd; the stream stops
        /// yielding new entries as soon as the drop returns, and any
        /// in-flight CQEs already queued in the inbox are dropped
        /// with the stream.
        ///
        /// # Errors
        ///
        /// Returns a stream that yields `Some(Err(Unsupported))` on
        /// first poll if the caller is not on a uring-reactor worker.
        /// `-ENOBUFS` surfaces the same way: the consumer is not
        /// releasing leases fast enough and the kernel ran out of
        /// buffers.
        ///
        /// # Platform support
        ///
        /// Linux kernel 6.0+ (the `SINGLE_ISSUER | DEFER_TASKRUN |
        /// COOP_TASKRUN` setup combined with multishot recv and
        /// provided buffer rings).
        #[cfg_attr(docsrs, doc(cfg(all(tokio_unstable, feature = "io-uring-reactor", target_os = "linux"))))]
        pub fn uring_recv_multi(&self) -> UringRecvMulti {
            use std::os::fd::AsRawFd;
            if !local_reactor_installed() {
                return UringRecvMulti::already_complete_err(io::Error::new(
                    io::ErrorKind::Unsupported,
                    "uring_recv_multi requires a tokio worker with the \
                     io-uring-reactor enabled",
                ));
            }
            let fd = self.as_raw_fd();
            ...
            let submit = with_local_reactor(|reactor| {
                reactor.submit_recv_multi(fd, inbox.clone())
            })
            .expect("local reactor vanished between probe and submit");
            match submit {
                Ok(_user_data) => UringRecvMulti::new(inbox, fd),
                Err(err) => UringRecvMulti::already_complete_err(err),
            }
        }
    }
}
