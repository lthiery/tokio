...
pub(crate) struct WorkerMetrics {
    ...
    #[cfg(tokio_unstable)]
    /// If `Some`, tracks the distribution of scheduling latencies.
    pub(super) scheduling_time_histogram: Option<Histogram>,
}
    ...
    cfg_metrics_variant! {
        ...
        unstable: {
            pub(crate) fn from_config(config: &Config) -> WorkerMetrics {
                ...
                worker_metrics.scheduling_time_histogram = config
                    .metrics_scheduling_time_histogram
                    .as_ref()
                    .map(|histogram_builder| histogram_builder.build());
                worker_metrics
            }
