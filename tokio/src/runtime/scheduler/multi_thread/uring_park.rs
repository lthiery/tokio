...
//! # Timer integration
//!
//! The ring shares the traditional single-mutex timer wheel. The ring
//! holder computes its `io_uring_enter` timeout as
//! `min(scheduler_timeout, time_until_next_timer)` via
//! [`compute_traditional_timer_duration`] before parking, then advances
//! the wheel via [`process_traditional_timer_after_park`] after wake.
//! Condvar parkers leave timers to the holder.
//!
...
use crate::runtime::io::uring_driver::{
    compute_traditional_timer_duration, process_traditional_timer_after_park, set_current_worker,
    UringHandle,
};
...
pub(crate) use crate::runtime::io::uring_driver::clear_current_worker;

/// Publish the current worker index from the worker's `run` entry point,
/// before any task executes on this thread. `GlobalRing::push_op` reads
/// it to tell worker pushers (whose own next park drains the op queue)
/// from external pushers (which may need a kick). Installing it lazily
/// (say, on first park) would misclassify ops pushed by tasks that
/// register fds before the worker's first park, common at startup.
///
...
impl UringParker {
    ...
    ///
    /// Timer split: the ring holder parks with the timer-min'd deadline
    /// and processes the wheel after waking (it is "the driver" in the
    /// stock sense); condvar parkers use the raw scheduler timeout and
    /// leave timers to the holder.
    fn park_global(&mut self, driver: &driver::Handle, duration: Option<Duration>) -> HadDriver {
        let g = Arc::clone(self.handle.global_ring());
        let driver_duration = compute_traditional_timer_duration(driver, duration);
        let drove_ring = g.park_worker(self.idx, driver_duration, duration);
        // Release ScheduledIos queued for drop and advance the timer
        ...
        process_traditional_timer_after_park(driver);
...
impl UringUnparker {
    ...
    /// target worker itself: short-circuiting on a current-worker check
    /// is unsound, because `transition_to_parked` can pop the calling
    /// worker off the sleepers list and invoke its own unparker; skipping
    /// the state transition there deadlocks the runtime.
