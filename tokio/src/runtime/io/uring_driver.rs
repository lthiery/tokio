...
impl GlobalRing {
    ...
    /// Timer-insert wake: the traditional time driver inserted a timer that
    ...
    /// min'd in the traditional timer deadline); `condvar_duration` is the raw
...
impl Drop for GlobalRing {
    /// Flush the op FIFO at runtime shutdown so queued register/deregister
    /// ops are applied (and their `Arc<ScheduledIo>`s released) rather than
    /// leaked in the queue, then drain any in-flight `tokio::fs` ops so
    /// owned buffers / fds are released and no CQE is left un-reaped. Runs
    /// after worker threads have joined (the runtime joins them before
    /// dropping the scheduler handle that owns the `UringHandle`), so
    /// `try_lock` on the reactor always succeeds here.
    fn drop(&mut self) {
        if let Some(mut reactor) = self.reactor.try_lock() {
            // Flush the FIFO first so every queued fs SQE is staged (a
            // `SubmitFsOp` may still be sitting here if the owning `Op`
            // was created but never yet drained by a holder).
...
/// Hybrid park flow helper: traditional timer + uring I/O.
///
/// When a uring-backed runtime is built without `enable_alt_timer()` (the
/// default), rings share the traditional single-mutex timer wheel. To
/// make
...
pub(crate) fn compute_traditional_timer_duration(
...
/// Mirror of [`compute_traditional_timer_duration`] for the post-park path:
/// process expired timers under the traditional flavor.
pub(crate) fn process_traditional_timer_after_park(driver: &crate::runtime::driver::Handle) {
    #[cfg(feature = "time")]
    if let Some(time_handle) = driver.time_handle_opt() {
        if time_handle.is_traditional() {
...
impl UringHandle {
    ...
    ///
    /// Errors surface from ring construction (a kernel without io_uring
    /// support, or resource limits); the runtime builder propagates them
    /// from `Builder::build`.
    pub(crate) fn new(num_workers: usize) -> io::Result<Self> {
        ...
        // thread (legal: the ring is not submitter-bound).
        let global = Arc::new(GlobalRing::new(num_workers)?);
        Ok(Self {
            global,
            registrations,
    ...
    /// minimum is honored. Called by the traditional time driver's insert path
    ...
    /// `UringParker::compute_traditional_timer_duration`). One woken worker is
    ...
    ///
    /// `Ok` means the registration is accepted and queued: readiness will
    /// be delivered to `shared` once the ring holder submits the SQE. A
    /// failure after this point (kernel rejects the POLL_ADD) surfaces by
    /// shutting down the `ScheduledIo` so waiters observe an error
    /// instead of waiting forever.
    ///
    /// NOTE: this queued interpretation is WIDER than the
    /// `IoDriverBackend` contract, which pins `Ok` to completed
    /// kernel-side registration. This backend's proposal depends on
    /// lazy first-poll registration (where the deferred error surfaces
    /// from the awaited poll) or an explicit contract widening; see the
    /// tracking issue's open question on accepted-vs-completed
    /// registration.
    ...
    // the fs side-driver exposes on `runtime::io::Handle`, so the upstream
    // `Op<T>` future drives either backend unchanged. Submission routes
    // through the shared-ring op FIFO (the future's thread may not be the
    // ring holder); completion is delivered by the holder's
    ...
    /// pass).
    pub(crate) fn deregister_source(&self, io: &Arc<ScheduledIo>) -> io::Result<()> {
...
impl super::io_driver::IoDriverBackend for UringHandle {
    ...
    fn register_local(
        &self,
        shared: &Arc<ScheduledIo>,
        source: &mut dyn super::registration::RegistrationSource,
        interest: Interest,
    ) -> io::Result<()> {
        // The reactor registers poll interest by fd; a source that is not
        // fd-backed (FreeBSD poll_aio is the known case, not reachable on
        // linux) cannot be driven by this backend and must fail here,
        // deterministically, rather than hang.
        let fd = source.registration_raw_fd().ok_or_else(|| {
            io::Error::new(
                io::ErrorKind::InvalidInput,
                "the io_uring reactor requires an fd-backed I/O source",
            )
        })?;
        self.register_local(shared, fd, interest)
    }

    fn deregister(
        &self,
        io: &Arc<ScheduledIo>,
        _source: &mut dyn super::registration::RegistrationSource,
    ) -> io::Result<()> {
        // The kernel-side POLL_REMOVE is keyed on the slab identity, not
        // the source: mio needs the source for Registry::deregister, this
        // backend does not.
        self.deregister_source(io)
    }
}
...
mod tests {
    ...
    fn new_builds_shared_ring() {
        let handle = UringHandle::new(4).unwrap();
    ...
    fn unpark_idle_worker_is_noop() {
        let handle = UringHandle::new(2).unwrap();
