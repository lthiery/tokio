...
impl Handle {
    ...
    /// fs side-driver's half of the backend-agnostic `Op<T>` protocol; the
    /// uring reactor exposes the same method (`UringHandle::poll_op`). The
    /// body is the relocated `Op::poll` `State::Polled` logic.
    pub(crate) fn poll_op(&self, index: usize, waker: &Waker) -> Option<io_uring::cqueue::Entry> {
