...
//!
//! Scope: readiness model for fd-backed resources, same semantics as the
//! mio driver, plus (with the `fs` feature) the in-tree `tokio::fs`
//! completion ops, which ride the same ring so a uring-reactor runtime
//! has one event system.
//!
//! # `user_data` encoding
//!
...
//!     - `VARIANT_FS_OP` (`fs` feature): single-shot `tokio::fs`
//!       completion op, keyed into the separate `FsOpSlab`.
//!
...
impl Reactor {
    ...
    /// Shared fs-op slab, cloned into the [`GlobalRing`] at construction so
    /// the op path (`register_op` / `poll_op` / `cancel_op`) never needs the
    /// ring `TryLock`.
    ///
    /// [`GlobalRing`]: super::uring_driver::GlobalRing
    #[cfg(feature = "fs")]
    ...
    /// fs side-driver's `UringContext::drop`: flush staged SQEs, drop
    /// already-completed slots, then block for the terminal CQE of each op
    /// still in the kernel (delivering it, which frees the slot and closes
    /// a delivered Open fd).
    ///
