...
//! slab index, exactly like the legacy side-driver's `UringContext.ops`
//! (`runtime::io::driver::uring`). The difference is purely transport: the
//! legacy driver owns its own ring and submits/reaps inline, while here the
...
//! the legacy driver's, so the `Op<T>` future, `Completable`, `Cancellable`,
...
impl FsOpSlab {
    ...
    /// the stored waker and returns `None`. Mirrors the legacy driver's
    ...
    /// (closing a delivered fd for the `Open` case). Mirrors the legacy
    /// driver's `Handle::cancel_op`.
    ...
    /// (closing a delivered fd for the `Open` case). Mirrors the legacy
    /// driver's `dispatch_completions`. Called by the ring holder from
    ...
    /// release here. Mirrors the legacy `UringContext::drop`'s `retain`.
