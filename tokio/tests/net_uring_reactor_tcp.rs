...
//! io_uring reactor (one shared ring, multi-thread scheduler).
//!
...
//!   `IoDriver::register_local` (uring backend) -> `PendingOp::Register` ->
//!   ring holder drains the queue at park entry -> `Reactor::register`
//!   submits `POLL_ADD_MULTI` -> CQE wakes the task via `ScheduledIo`.
//!
...
fn tcp_multi_worker_round_trip() {
    ...
    // different workers, exercising the shared pending-ops queue from
    // multiple pushers while the ring rotates holders.
    let rt = build_rt(4);
    rt.block_on(async {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
        let addr = listener.local_addr().unwrap();
...
fn tcp_many_concurrent_connections() {
    // Exercise the pending-ops queue under fan-out: each connect creates
    // a fresh fd which is registered via `PendingOp::Register`, and all
    // four workers push while whoever holds the ring drains.
    const N: usize = 32;
    let rt = build_rt(4);
    rt.block_on(async {
        let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
