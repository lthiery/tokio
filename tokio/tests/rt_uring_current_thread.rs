...
//!
//! `enable_uring_reactor()` on `new_current_thread()` builds the same
//! single shared relaxed ring as the multi-thread flavor (no
//! `SINGLE_ISSUER`/`DEFER_TASKRUN`), with one worker park slot, driven
//! by whichever thread holds the scheduler core.
//!
...
/// eventfd), not the never-polled mio waker. Pre-wiring this hung: the
/// wake vanished and `block_on` slept forever.
#[test]
fn remote_spawn_wakes_parked_runtime() {
    ...
    // wake is an indefinite park, hence the watchdog.
...
/// net I/O, the strategic point of current_thread support.
#[test]
...
/// `block_on` caller that lost the core race polls its future (arming
/// its `Sleep`) on the `Notify` fallback path. That insert must kick
/// the holder's ring (`unpark_for_insert` -> `uring_handle()` ->
/// `timer_kick`); the mio unpark would land on a driver nobody polls
/// and BOTH threads would sleep forever (hence the watchdog).
#[test]
