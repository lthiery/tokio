...
//! completions).
//!
//! Unlike `fs_uring*.rs` (which drive the fs side-driver's own ring on a
//! mio runtime), every runtime here is built with
...
/// The same round-trip on a current_thread reactor runtime (the
/// single-ring, one-slot park path), so both scheduler flavors are
/// covered.
#[test]
